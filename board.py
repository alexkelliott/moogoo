...
class Board():
	...
	def __init__(self, players, game_state) -> None:
		...
		self.bets: dict[Suit, list[Fruit]] = {}
		for suit in Suit:
			self.bets[suit] = []

		self.top_card: dict[Suit, Card] = {}
		for suit in Suit:
	...
	def lowest_suit(self) -> Suit:
		top_card_list: list[Card] = [c for c in self.top_card.values() if c]
	...
	def reset(self) -> None:
		...
		lowest_suit: Suit = self.lowest_suit()
		if lowest_suit:
		...
		for player in self.players:
			cards: list[Card] = random.sample(self.deck, 5 - len(player.hand))
	...
	def round_complete(self) -> bool:
		top_card_list: list[Card] = [c for c in self.top_card.values() if c]
		full_cards: bool = (len(top_card_list) == 6 - len(self.removed_suits))
	...
	def final_scores(self) -> list[Player]:
	...
	def get_card(self, player) -> None:
		...
		choice: Card = random.choice(self.deck)
		if choice:
	...
	def handle_bet_selection(self) -> Suit:
		...
		if player.is_human: # Human player's turn
			...
			available: list[Suit] = [suit for suit in self.bets if suit not in self.removed_suits and len(self.bets[suit]) < 4] # find spots with < 4 bets
	...
	def get_hovered_card(self) -> Card:
		hand: list[Card] = self.players[self.turn].hand

		for i in range(len(hand)):
			if hand[i]:
				bound: dict[str, int] = self.card_boundaries[i]
				if bound["left"] <= self.game_state.mouse_coords['x'] <= bound["right"] and bound["top"] <= self.game_state.mouse_coords['y'] <= bound["bottom"]:
					return hand[i]
	...
	def handle_card_selection(self) -> Card:
		...
		if choice:
			...
			if choice.rank == 'q':
				new_val: int = random.randint(1,7)
